...
class Role {
    ...
    handleNotifyRendezvousNotReady(msg) {
        var rid = msg['rendezvous_id'];
        console.info("waiting for peer to rendezvous");
        this.setState("RENDEZVOUS_SETUP");

        if ("NOTIFY_RENDEZVOUS_NOT_READY" in this.hooks) {
            this.hooks['NOTIFY_RENDEZVOUS_NOT_READY'](msg, this);
        }
    }
    ...
    handleNotifyProviderNotReady(msg) {
       // if (this.state != "PROVIDER_SETUP") {
       //     console.error("not in provider setup state")
       //     // TODO do we notify on error?
       //     return;
       // }
        if ("NOTIFY_PROVIDER_NOT_READY" in this.hooks) {
            this.hooks['NOTIFY_PROVIDER_NOT_READY'](msg, this);
        }
    }
    ...
    handleNotification(msg) {
        ...
        } else if (n == "NOTIFY_RENDEZVOUS_NOT_READY") {
            this.handleNotifyRendezvousNotReady(msg);
        ...
        } else if (n == "NOTIFY_PROVIDER_NOT_READY") {
            this.handleNotifyProviderNotReady(msg);
